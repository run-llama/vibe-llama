...
class RAGWorkflow(Workflow):
    @step
    async def document_processing_step(self, ev: StartEvent, ctx: Context) -> IndexCreatedEvent:
    ...
    @step
    async def retrieve_step(self, ev: IndexCreatedEvent, ctx: Context) -> RetrievalEvent:
        state = await ctx.store.get_state()
        retrieved_documents = await ev.index.as_retriever(top_k=5).aretrieve(state.query)
        return RetrievalEvent(documents=retrieved_documents)

    @step
    async def generate_step(self, ev: RetrievalEvent, llm: Annotated[LLM, Resource(get_llm)], ctx: Context) -> StopEvent:
        state = await ctx.store.get_state()
        docs = "\\n\\n---\\n\\n".join([f"Content: {node.text}\\nScore: {node.score if node.score else -1}" for node in ev.documents])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--path", help="Path to the directory with the files to ingest", required=True)
