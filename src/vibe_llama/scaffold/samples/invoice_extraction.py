...
class InvoiceExtractWorkflow(Workflow):
    @step
    async def invoice_extraction(self, ev: StartEvent, ctx: Context, extractor: Annotated[LlamaExtract, Resource(get_invoice_extractor)]) -> FeedbackRequiredEvent:
        ...
        result = await extractor.aextract(data_schema=InvoiceData, config=config, files=[ev.path])
        ...
        async with ctx.store.edit_state() as state:
            state.extraction_result = "\\n\\n---\\n\\n".join([f"Invoice Date: {d.invoice_date}\\nCustomer: {d.customer}\\nAmount Due: {d.amount_due}" for d in extracted_data])
        return FeedbackRequiredEvent(extraction_result="\\n\\n---\\n\\n".join([f"Invoice Date: {d.invoice_date}\\nCustomer: {d.customer}\\nAmount Due: {d.amount_due}" for d in extracted_data]))

    @step
    async def human_feedback(self, ev: HumanFeedbackEvent, ctx: Context) -> StopEvent | StartEvent:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--path", required=True, help="Path to the invoice to extract")
    parser.add_argument("-m", "--mode", required=True, help="Extraction mode", choices=["base", "advanced", "premium"])
