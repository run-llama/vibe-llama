...
class WebScrapeWorkflow(Workflow):
    @step
    async def process_urls(self, ev: StartEvent, ctx: Context[URLState]) -> URLReadEvent | None:
    ...
    @step
    async def get_url_content(self, ev: URLReadEvent, llm: Annotated[GoogleGenAI, Resource(get_llm)], ctx: Context[URLState]) -> URLContentEvent:
        response = llm.chat([ChatMessage(role="user", content=f"Can you please summarize the context of this URL: {ev.url}")])
        async with ctx.store.edit_state() as state:
            state.final_content += f"### Summary for {ev.url}\\n\\n{response.message.content}\\n\\n"
        return URLContentEvent(content=response.message.content or "")

    @step
    async def finalize(self, ev: URLContentEvent, ctx: Context[URLState]) -> StopEvent | None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--url", help="URLs whose content needs to be summarised", required=True, action="append")
    args = parser.parse_args()

    if not os.getenv("GOOGLE_API_KEY", None):
        raise ValueError("You need to set GOOGLE_API_KEY in your environment before using this workflow")
